...
export const metadata: Metadata = {
  title: "Dashboard | Best Aircraft Detailer",
};
...
async function getOverviewStats(companyId: string): Promise<{
  ...
  try {
    ...
    return { locationCount: 2, photoCount: 5, leadCount: 8 };
  }
}
...
async function getCompany(): Promise<Company | null> {
  try {
    ...
    return {
      ...
      description: "White-glove aircraft detailing for discerning owners.",
  ...
  if (!company) {
    return (
      <div className="border border-ivory-200 bg-white py-16 text-center rounded-sm">
        <svg
          className="mx-auto h-12 w-12 text-ivory-400"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          strokeWidth={1}
        ...
        <p className="mt-4 text-ivory-500">
          No business listing linked to your account.
        </p>
        <Link
          href="/search"
          className="mt-4 inline-block text-sm font-semibold text-gold-500 transition-colors hover:text-gold-600"
        >
    ...
    <div className="space-y-6">
      ...
      <div className="border border-ivory-200 bg-white p-6 rounded-sm">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-heading text-sm font-semibold text-noir-900">
            Your Verified Listing
          </h2>
        </div>
        <p className="mt-2 font-heading text-xl font-semibold text-noir-900">
          {company.name}
        </p>
        <p className="mt-1 text-sm text-ivory-500">
          Tier:{" "}
          <span className="font-semibold capitalize text-noir-900">{company.tier}</span>
        </p>
      </div>

      {/* Trust score highlight */}
      <div className="border border-ivory-200 bg-white p-6 text-center rounded-sm">
        <p className="text-sm uppercase tracking-wider text-ivory-500 font-body">Trust Score</p>
        <p className="mt-1 font-heading text-4xl text-gold-500">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="border border-ivory-200 bg-white p-5 rounded-sm">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <Link
          ...
          className="group border border-ivory-200 bg-white p-6 transition-all hover:border-gold-500 rounded-sm"
        >
          <svg
          ...
          <h3 className="mt-4 font-heading text-sm font-semibold text-noir-900 group-hover:text-gold-500">
            View Public Profile
          </h3>
          <p className="mt-1 text-sm text-ivory-500">
        ...
        >
          ...
          <p className="mt-1 text-sm text-ivory-500">
            Manage your gallery ({stats.photoCount}/{features.maxPhotos === Infinity ? "Unlimited" : features.maxPhotos}).
          </p>
        </Link>
      </div>
    </div>
