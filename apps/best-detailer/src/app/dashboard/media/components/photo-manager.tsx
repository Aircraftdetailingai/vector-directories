...
import { addPhoto, deletePhoto, type ActionResult } from "../actions";
...
export function PhotoManager({ photos, companyId, tier }: PhotoManagerProps) {
  ...
  if (maxPhotos === 0) {
    return (
      <div className="border border-dashed border-ivory-300 bg-ivory-50 py-16 text-center rounded-sm">
        <svg
        ...
        <h3 className="mt-4 font-heading text-sm font-semibold text-noir-900">
          Photo Gallery
        </h3>
        ...
        <a
          ...
          className="mt-4 inline-block bg-noir-900 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-noir-800 rounded-sm"
        >
          View Plans
        </a>
      </div>
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end">
            ...
            <div className="flex-1">
              ...
              <input
                ...
                className="mt-1 w-full border border-ivory-200 bg-white px-3 py-2 text-sm text-noir-900 placeholder-ivory-400 focus:border-gold-500 focus:outline-none focus:ring-1 focus:ring-gold-500 rounded-sm"
              />
            </div>
            <button
              type="submit"
              disabled={isPending}
              className="flex-shrink-0 bg-noir-900 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-noir-800 disabled:opacity-50 rounded-sm"
            >
      ...
      {photos.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {photos.map((photo) => (
            <div
              key={photo.id}
              className="group relative overflow-hidden border border-ivory-200 bg-white rounded-sm"
            >
              ...
              >
                ...
                <button
                  type="submit"
                  disabled={isPending}
                  className="bg-white/90 p-1.5 text-red-600 shadow-sm transition-colors hover:bg-red-50 rounded-sm"
                  title="Delete photo"
                >
                  <svg
                    className="h-4 w-4"
              ...
              {photo.alt_text && (
                <p className="px-3 py-2 text-xs text-ivory-500">
                  {photo.alt_text}
                </p>
              )}
            </div>
