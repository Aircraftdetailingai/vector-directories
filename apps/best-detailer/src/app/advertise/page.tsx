...
export const metadata: Metadata = {
  title: "Advertise With Us | Best Aircraft Detailer",
  description:
    "Reach aircraft detailing decision-makers. Advertise on Best Aircraft Detailer.",
};
...
const PLACEMENTS = [
  ...
  },
  {
    ...
    description:
      "Boost your listing to the top of search results with a featured badge.",
  },
  {
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-1">
        {/* Hero */}
        <section className="bg-noir-900 py-16 sm:py-24">
          <div className="mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
            <h1 className="font-heading text-4xl font-bold text-gold-500 sm:text-5xl tracking-wide">
              Advertise With Us
            </h1>
            <p className="mx-auto mt-4 max-w-2xl text-lg text-ivory-400">
        ...
        {/* Stats */}
        <section className="border-b border-ivory-200 bg-ivory-50 py-12">
          <div className="mx-auto grid max-w-4xl grid-cols-1 gap-8 px-4 sm:grid-cols-3 sm:px-6 lg:px-8">
            {STATS.map((stat) => (
              <div key={stat.label} className="text-center">
                <p className="font-heading text-3xl font-bold text-noir-900">
                  {stat.value}
                </p>
        ...
        <section className="bg-ivory-50 py-16">
          <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            <h2 className="font-heading text-center text-2xl font-bold text-noir-900 tracking-wide">
            ...
            <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {PLACEMENTS.map((p) => (
                <div
                  key={p.type}
                  className="flex flex-col border border-ivory-200 bg-white p-6 shadow-sm transition-shadow hover:shadow-md"
                >
                  <h3 className="font-heading text-lg font-bold text-noir-900">
                    {p.name}
                  </h3>
                  ...
                  <p className="mt-3 flex-1 font-body text-sm text-gray-600">
                    {p.description}
                  </p>
                  <Link
                    href={`/advertise/checkout?placement=${p.type}`}
                    className="mt-6 block bg-noir-900 px-4 py-2.5 text-center text-sm font-semibold text-white transition-colors hover:bg-noir-800"
                  >
                    Get Started
                  </Link>
                </div>
        ...
        <section className="bg-white py-16">
          <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
            ...
            <dl className="mt-10 space-y-8">
              {FAQS.map((faq) => (
                <div key={faq.q}>
                  <dt className="font-heading text-base font-semibold text-noir-900">
                    {faq.q}
                  </dt>
                  <dd className="mt-2 font-body text-sm text-gray-600">{faq.a}</dd>
                </div>
              ))}
            </dl>
          </div>
        ...
        <section className="bg-noir-900 py-16">
          <div className="mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
            <h2 className="font-heading text-3xl font-bold text-gold-500 tracking-wide">
              Ready to Grow Your Business?
            </h2>
            <p className="mx-auto mt-4 max-w-xl text-ivory-400">
              Start advertising today and connect with aircraft detailing
              professionals across the country.
            </p>
            <Link
              href="/advertise/checkout?placement=top_banner"
              className="mt-8 inline-block bg-gold-500 px-8 py-3 text-sm font-semibold text-noir-900 transition-colors hover:bg-gold-600"
            >
              Get Started Now
            </Link>
          </div>
