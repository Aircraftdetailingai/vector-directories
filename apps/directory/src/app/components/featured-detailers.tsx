...
import { getTrustScoreLabel, getTrustScoreColor, formatTrustScore } from "@vector/utils";

interface FeaturedDetailersProps {
  companies: Company[];
}
...
export function FeaturedDetailers({ companies }: FeaturedDetailersProps) {
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex items-end justify-between">
          <div>
            ...
            <p className="mt-3 text-gray-600">
              Top-rated aircraft detailing companies
            </p>
          </div>
          <Link
            href="/browse/a-z"
        ...
        <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {companies.map((company) => {
            const color = getTrustScoreColor(company.trust_score);
            const colorClass = SCORE_BG[color] ?? SCORE_BG.gray;

            return (
              <Link
                key={company.id}
                href={`/company/${company.slug}`}
                className="group rounded-xl border border-gray-200 bg-white p-6 shadow-sm transition-all hover:border-forest-300 hover:shadow-lg hover:shadow-forest-100/50"
              >
