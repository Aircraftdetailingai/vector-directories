...
export default function TierComparison({
  ...
  return (
    <>
      {/* Desktop table */}
      <div className="hidden overflow-hidden rounded-xl border border-brand-100 lg:block">
        <table className="min-w-full divide-y divide-brand-100">
          <thead>
            <tr>
              <th className="bg-cream-50 px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-brown-500">
                Feature
              </th>
              {TIERS.map((t) => (
                <th
                  ...
                  className={`px-6 py-4 text-center text-xs font-semibold uppercase tracking-wider ${
                    t.key === currentTier
                      ? "bg-brand-50 text-brand-600 ring-2 ring-inset ring-brand-500"
                      : "bg-cream-50 text-brown-500"
                  }`}
                >
                  <div>{t.name}</div>
                  <div className="mt-1 text-lg font-bold normal-case tracking-normal text-brown-500">
                    {t.price}
                  </div>
                </th>
              ))}
          ...
          <tbody className="divide-y divide-brand-100 bg-white">
                  ...
                  return (
                    <td
                      ...
                      className={`px-6 py-3 text-center text-sm ${
                        t.key === currentTier ? "bg-brand-50/50" : ""
                      } ${val === "Yes" ? "font-semibold text-green-700" : val === "\u2014" ? "text-gray-300" : "text-gray-700"}`}
                    >
                      {val}
                    </td>
                  );
            ...
            <tr>
              ...
              {TIERS.map((t) => (
                <td
                  key={t.key}
                  className={`px-6 py-4 text-center ${t.key === currentTier ? "bg-brand-50/50" : ""}`}
                >
                  {t.key === "basic" ? (
                    t.key === currentTier ? (
          ...
          return (
            <div
              ...
              className={`rounded-xl border p-6 ${
                isCurrent
                  ? "border-brand-300 bg-brand-50 ring-2 ring-brand-500"
            ...
            >
              <div className="flex items-center justify-between">
                ...
                {isCurrent && (
                  <span className="rounded-xl border border-brand-200 bg-brand-100 px-3 py-1 text-xs font-semibold text-brand-600">
                    Current
                  </span>
                )}
              </div>
                ...
                {FEATURE_ROWS.map((row) => {
                  ...
                  return (
                    <div key={row.label} className="flex justify-between">
                      ...
                      <dd
                        className={`text-sm font-medium ${val === "Yes" ? "text-green-700" : val === "\u2014" ? "text-gray-300" : "text-gray-700"}`}
                      >
                        {val}
                      </dd>
                    </div>
