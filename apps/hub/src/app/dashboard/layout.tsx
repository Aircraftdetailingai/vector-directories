...
async function getDashboardData(userId: string): Promise<{
  ...
  try {
    ...
    if (!profile?.company_id) {
      return { userProfile: profile as unknown as UserProfile, company: null };
    }

    const company = await getCompanyById(client, profile.company_id);
    return { userProfile: profile as unknown as UserProfile, company };
  } catch {
    return {
      userProfile: {
        id: "00000000-0000-0000-0000-000000000099",
        email: "alex@skyshine-aviation.example.com",
        ...
        <section className="bg-teal-600 py-8">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            {userProfile?.full_name && (
              <p className="mt-1 text-sm text-teal-100 font-body">
                Welcome back, {userProfile.full_name}! Great to see you.
              </p>
            )}
          </div>
        </section>
