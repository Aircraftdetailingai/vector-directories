...
import { TIER_FEATURES, getTrustScoreLabel } from "@vector/utils";

export const metadata: Metadata = {
  title: "Hub Dashboard | Aviation Detailing Hub",
};
...
async function getOverviewStats(companyId: string): Promise<{
  ...
  try {
    ...
    return { locationCount: 1, photoCount: 3, leadCount: 12 };
  }
}
...
export default async function DashboardOverviewPage() {
  ...
  if (!company) {
    return (
      <div className="rounded-2xl border border-teal-100 bg-white py-16 text-center">
        <svg
        ...
        <p className="mt-4 text-gray-500 font-body">
          No business listing linked to your account yet.
        </p>
        <Link
          href="/search"
  ...
  const trustLabel = getTrustScoreLabel(company.trust_score);
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-teal-200 bg-teal-50 p-6">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-heading text-sm font-semibold text-teal-800">
            Your Verified Hub Listing
          </h2>
        </div>
        <p className="mt-2 font-heading text-xl font-bold text-teal-900">
          {company.name}
        </p>
        <div className="mt-1 flex flex-wrap items-center gap-3 text-sm text-teal-600 font-body">
          <span>
            Tier:{" "}
            <span className="font-semibold capitalize">{company.tier}</span>
          </span>
          <span>&middot;</span>
          <span>
            Trust Score:{" "}
          ...
          <span
            className="inline-flex items-center rounded-full bg-coral-100 px-2 py-0.5 text-xs font-semibold text-coral-700"
          >
            {trustLabel}
          </span>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="rounded-2xl border border-teal-100 bg-white p-5">
          <p className="text-2xl font-bold text-teal-700 font-heading">
            {stats.locationCount}
          </p>
          <p className="mt-1 text-sm text-gray-500 font-body">
            Location{stats.locationCount !== 1 ? "s" : ""}{" "}
            <span className="text-gray-400">
              / {features.maxLocations === Infinity ? "Unlimited" : features.maxLocations}
            </span>
          </p>
        </div>
        <div className="rounded-2xl border border-teal-100 bg-white p-5">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <Link
          ...
          className="group rounded-2xl border border-teal-100 bg-white p-6 transition-all hover:border-teal-300 hover:shadow-lg hover:shadow-teal-100/50"
        >
          <svg
          ...
          <h3 className="mt-4 font-heading text-sm font-semibold text-gray-900 group-hover:text-teal-700">
            View Public Profile
          </h3>
          <p className="mt-1 text-sm text-gray-500 font-body">
            See how your listing appears to the community.
          </p>
        </Link>
        ...
        >
          ...
          <p className="mt-1 text-sm text-gray-500 font-body">
            Showcase your work ({stats.photoCount}/{features.maxPhotos === Infinity ? "Unlimited" : features.maxPhotos}).
          </p>
        </Link>
      </div>
    </div>
