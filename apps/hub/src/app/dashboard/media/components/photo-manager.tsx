...
import { uploadPhoto, removePhoto, type ActionResult } from "../actions";
...
export function PhotoManager({ photos, companyId, tier }: PhotoManagerProps) {
  ...
  function handleDelete(formData: FormData) {
    startTransition(async () => {
      const res = await removePhoto(formData);
      setResult(res);
    });
  }
  ...
  if (maxPhotos === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-teal-200 bg-teal-50/50 py-16 text-center">
        <svg
          className="mx-auto h-12 w-12 text-teal-300"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          strokeWidth={1}
        ...
        <p className="mt-2 text-sm text-gray-500 font-body">
          Upgrade to Enhanced or above to showcase photos of your work to the community.
        </p>
        <a
          href="/dashboard/upgrade"
          className="mt-4 inline-block rounded-full bg-coral-400 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-coral-500 font-body"
        >
          View Plans
        </a>
      </div>
        ...
        >
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end">
            ...
            <button
              type="submit"
              disabled={isPending}
              className="flex-shrink-0 rounded-full bg-teal-600 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-teal-700 disabled:opacity-50 font-body"
            >
      ...
      {photos.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {photos.map((photo) => (
            <div
              key={photo.id}
              className="group relative overflow-hidden rounded-2xl border border-teal-100 bg-white"
            >
              ...
              >
                ...
                <button
                  type="submit"
                  disabled={isPending}
                  className="rounded-full bg-white/90 p-1.5 text-red-600 shadow-sm transition-colors hover:bg-red-50"
                  title="Delete photo"
                >
                  <svg
                    className="h-4 w-4"
              ...
              {photo.alt_text && (
                <p className="px-3 py-2 text-xs text-gray-500 font-body">
                  {photo.alt_text}
                </p>
              )}
            </div>
